...
*   **Event Sources (both implemented):** `USGSSource` (Secondary) and `METSource` (Primary — the Pakistan MET Department feed at `weather.gov.pk`) both implement the `SeismicSource` protocol in `src/eqmon/events/sources.py`. MET wins dedup as canonical for shared quakes. The PMD feed is **HTTPS bearer-authenticated** via the `PMD_API_URL`/`PMD_API_TOKEN` env vars (the token is never sent over a non-HTTPS scheme), returns its **full catalog every call** (no incremental params), and is **dirty** — `parse_met()` defensively parses hemisphere-suffixed coordinates and sanity-bounds magnitude/depth, skipping unparseable/out-of-range rows. Both are ingested on a scheduler (MET first) and via manual endpoints (`POST /events/ingest`, `POST /events/ingest/met`).
*   **Exposure (ARC) band semantics:** `src/eqmon/exposure.py` proxies the external ARC elements-at-risk service. Two traps it guards, both load-bearing: ARC dissolves bands on `mmi_high` (our name is `mmi_upper`) — translated only in `to_arc_bands()`; and ARC keys `cumulative[].mmi_min` on a band's **upper** bound, so its "MMI ≥ 6" row includes the 5–6 band that we label MMI V. The headline is summed from the bands, never read from `cumulative`. Complements `impact.py` (which admin units shake) rather than replacing it.
*   **Intensity catalog persistence is opt-in:** `POST /intensity` only saves a MANUAL event when the request sets `save_to_catalog: true` (surfaced as a checkbox in the UI) — bands always render regardless.