...
- `web/maplibre-3d.js`
- `web/buildings-config.js` (new: bands and viewport rules shared by both renderers)
- `web/buildings.js`
- `web/styles.css`
...
- [x] Reuse `GET /buildings/districts` for bounds and minimum zoom.
- [x] Reuse `/buildings/tiles/{district}/{z}/{x}/{y}.pbf` sources.
- [x] Add/remove district vector sources according to viewport intersection,
  preserving the current rule that only a small set is live at once.
- [x] Use `fill-extrusion-height` from positive finite `height` values in metres.
- [x] Give unknown/non-positive heights a small flat height rather than implying a
  measured building height.
- [x] Reuse the five height-band colors and shared opacity.
- [x] Keep buildings below MMI and event emphasis in the visual hierarchy.
- [x] Disable extrusion below the configured minimum zoom.
- [x] Update district selection and map framing through the mode coordinator.
- [x] Add a configurable source cap and eviction policy for long pan sessions.

**Acceptance criteria**

- [x] Building heights and colors match the existing legend.
- [x] Selecting a district frames it and renders at a usable zoom in both modes.
- [x] Unknown heights are visually distinct and never interpreted as high-rise.
- [x] Panning across districts does not leave stale sources or unbounded memory.
...
- [x] Add each enabled landslide raster PMTiles archive as an independent source.
- [x] Preserve AJK, GB, KP, and Balochistan toggles and shared opacity.
- [x] Preserve transparent/unassessed areas and the Very High-only classification.
- [x] Port PGA return-period raster sources, class breaks, opacity, and attribution.
- [x] Drape raster hazards over terrain without smoothing categorical classes.
- [x] Use nearest-neighbor raster resampling where MapLibre supports it.
- [x] Keep landslide and PGA legends mode-independent so switching renderers does
  not duplicate or reposition them.
- [x] Report archive/source errors per layer without disabling unrelated regions.

**Acceptance criteria**

- [x] All four landslide regions can be toggled independently in 3D.
- [x] Landslide and PGA colors, opacity, and legends match 2D.
- [x] Raster edges align with administrative reference geometry.
- [x] One failed archive does not remove other active hazard layers.

**Phase 7 findings — 2026-08-27**

- Each regional landslide archive is one persistent MapLibre raster source and
  layer. Region toggles change `visibility`, opacity edits change paint in place,
  and PMTiles PNG transparency preserves unassessed cells and the filtered Very
  High-only classification generated by `scripts/build_landslide_tiles.py`.
- PGA uses the same classified PNG, geographic bounds, return-period selection,
  opacity, and mode-independent legend as 2D. Changing period replaces only the
  single image source; switching map mode performs no analytical request.
- Both hazard types use nearest-neighbor resampling and zero fade, and are
  inserted below MMI so categorical edges stay crisp while draping over terrain.
- Source failures are announced once per source and do not remove or hide any
  sibling landslide region or the other hazard family.
- The MapLibre harness covers all four independent regional layers, opacity and
  visibility updates without source growth, PGA image coordinates, hierarchy,
  nearest-neighbor paint, and restoration after a complete style swap.
...
- [x] Preserve selected event, selected MMI band, filters, and open sidebar section.
- [x] Ensure time-window controls update 3D event visibility.
- [x] Keep map legends clear of the sidebar, MMI ladder, status bar, exposure strip,
  chat trigger, and MapLibre attribution/controls.
- [x] Keep toggle reachable and unobscured at 200% browser zoom.
- [x] Add visible focus, `aria-pressed`, descriptive group label, and live status
  text for loading/failure.
- [x] Do not announce routine camera changes to screen readers.
- [x] Respect reduced motion for camera transitions, epicenter pulse, and fades.
- [x] On mobile, default 3D bearing to north and provide a one-tap reset.
- [x] Disable browser context-menu conflicts only where MapLibre requires them.
- [x] Confirm expanded Forecast, Elements at Risk, and Insights restore the correct
  renderer and call its resize method when closed.

**Acceptance criteria**

- [x] Complete mode toggle is keyboard-operable and screen-reader named.
- [x] Layout works at 1440x900, 1024x768, 430x800, and 200% zoom.
- [x] Opening/closing every full-screen view leaves the active map correctly sized.
- [x] Reduced-motion users receive no animated fly or spin transition.

**Phase 8 findings — 2026-08-27**

- The coordinator now owns renderer suspension explicitly. Insights, expanded
  Forecast, and Elements at Risk suspend both renderers and the mode control,
  then restore the same active mode and resize both maps on close.
- The native 2D/3D buttons retain 44 px targets, visible focus, `aria-pressed`,
  and a descriptive "Map view mode" group name. One atomic live region announces
  loading, successful mode changes, and fallback; camera movement stays silent.
- Mobile 3D applies north-up bearing on entry. MapLibre's navigation compass is
  the one-tap reset, while touch rotation and touch pitch remain disabled so map
  gestures do not conflict with page scrolling.
- Event-window and bubble-opacity controls republish the complete filtered event
  snapshot, so 3D changes immediately without fetching or maintaining a second
  filter implementation.
- Responsive rules and browser coverage check the control at 1440x900, 1024x768,
  and 430x800; the high-resolution rule covers the effective narrow viewport at
  200% zoom. Reduced-motion removes renderer fades and the epicenter pulse.
...
- [x] Test pure camera conversion and shared-state normalization functions.
- [x] Test lazy initialization, one-instance invariant, fallback, and mode state.
- [x] Browser-test 2D -> 3D -> 2D camera and layer preservation.
- [x] Browser-test WebGL failure and DEM failure separately.
...
- [x] Run interaction tests in dark and light themes.
...
- [x] Pause hidden renderer animation/render loops where supported.
- [x] On low-memory/WebGL context loss, return to 2D and release recoverable 3D
...
**Phase 9 hardening update — 2026-08-27**

- The deterministic Node harness covers camera normalization and conversion,
  JSON state isolation, lazy one-instance initialization, twenty mode switches,
  suspension/resume, no-refetch behavior, and WebGL context-loss fallback.
- The MapLibre adapter exposes a narrow lifecycle seam: `setPaused()` stops the
  hidden renderer and resizes/repaints it on resume; `onRendererFailure()` reports
  context loss without leaking canvas or WebGL objects into the coordinator.
- A lost WebGL context returns immediately to the still-live Leaflet renderer.
  Camera and analysis state remain in the coordinator, so fallback does not
  reload the page or refetch analysis.
- Browser coverage now includes explicit context-loss fallback and hidden-renderer
  pause/resume in addition to DEM fallback, themes, camera, MMI, overlays, and
  buildings.
- Performance budgets and Edge/Chrome sign-off remain open because they require
  operations-room hardware and stable external tile/DEM access. Local browser
  runs remain intermittently blocked by aborted OpenFreeMap sprite requests.
